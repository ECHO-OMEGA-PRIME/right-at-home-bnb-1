...
assert(
  'Next production build treats TypeScript errors as fatal',
  nextConfig.includes('ignoreBuildErrors: false'),
);

const vercelConfig = read('apps/web/vercel.json');
let parsedVercel = null;
try {
  parsedVercel = JSON.parse(vercelConfig);
} catch (error) {
  failures.push(`Vercel configuration is valid JSON - ${error instanceof Error ? error.message : String(error)}`);
}
const invalidCredentialedWildcardCors = (parsedVercel?.headers ?? []).some((rule) => {
  const headers = Object.fromEntries(
    (rule.headers ?? []).map((header) => [String(header.key).toLowerCase(), String(header.value)]),
  );
  return (
...
assert(
  'Vercel does not combine wildcard CORS with credentialed requests',
  parsedVercel !== null && !invalidCredentialedWildcardCors,
);

/**
 * Right at Home BnB - iCal Export for VRBO
 * VRBO polls this endpoint every 60 minutes to sync our calendar
...
export async function GET(
  ...
  try {
    ...
    // Get all non-VRBO bookings for this property (to avoid duplication)
    ...
    for (const booking of bookings) {
      // Skip VRBO bookings to prevent circular sync
      if (booking.source === 'vrbo') continue;
    ...
    return new NextResponse(icalContent, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="property_${propertyId}_vrbo.ics"`,
    ...
    return new NextResponse(`Error generating calendar: ${error.message}`, {
      status: 500,
    });
  }
}

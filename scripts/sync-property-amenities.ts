 ...
 *
 * Usage: npx ts-node scripts/sync-property-amenities.ts
 */
...
interface ServiceFees {
  cleaningFee: number;
  deepCleanFee: number;
  poolServiceFee: number;
  lawnServiceFee: number;
}

interface WaiverRequirements {
  pool: boolean;
  hotTub: boolean;
  firePit: boolean;
  general: boolean;
...
interface PropertyJson {
  ...
  bathrooms?: Array<{ features?: string[] }>;
  spaces?: string[];
...
function detectAmenities(property: PropertyJson): {
  hasPool: boolean;
  hasHotTub: boolean;
  hasFirePit: boolean;
  hasGrill: boolean;
  ...
  const titleText = `${property.title || ''} ${(property as { tagline?: string }).tagline || ''}`;
...
function calculateServiceFees(property: PropertyJson, detected: ReturnType<typeof detectAmenities>): ServiceFees {
  ...
  const bedrooms = (property as { specs?: { bedrooms?: number } }).specs?.bedrooms || 3;
  if (bedrooms >= 5) {
...
function buildWaiverRequirements(detected: ReturnType<typeof detectAmenities>): WaiverRequirements {
  return {
...
async function syncPropertyFile(filePath: string): Promise<SyncResult> {
  ...
  try {
    ...
    const property: PropertyJson = JSON.parse(content);
    ...
    const updatedProperty: PropertyJson = {
      ...property,
      serviceFees,
      waiverRequirements,
...
async function findPropertyFiles(baseDir: string): Promise<string[]> {
...
async function main() {
  ...
  const propertyFiles = await findPropertyFiles(baseDir);
  ...
  for (const filePath of propertyFiles) {
    const result = await syncPropertyFile(filePath);
    results.push(result);
  ...
  console.log('\n========================================');
  console.log('  DATABASE RECORDS (Copy to seed file)');
  console.log('========================================\n');

  console.log('// PropertyServiceFees seed data');
  console.log('const propertyServiceFees = [');
  for (const result of results.filter(r => r.updated)) {
    const fees = result.hasPool
      ? '{ cleaningFee: 150, deepCleanFee: 300, poolServiceFee: 50, lawnServiceFee: 75 }'
